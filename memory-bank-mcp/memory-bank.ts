...
server.tool(
  "mcp_memory_bank_batch_operations",
  "Performs batch operations on nodes and edges in a single transaction.",
  {
    project_name: z.string().min(1).describe("Name of the project"),
    nodes: z.array(BatchNodeSchema).optional().describe("Array of nodes to add or update"),
    edges: z.array(BatchEdgeSchema).optional().describe("Array of edges to add"),
    operation_type: z.enum(["add"]).default("add").describe("Type of batch operation (currently only 'add' is supported)"),
  },
        ...
        for (const edge of edges) {
          try {
            ...
            graph.addDirectedEdge(
              edge.sourceId, 
              edge.targetId, 
              { relationshipType: edge.relationshipType } as GraphEdgeAttributes
            );
            results.edgesAdded++;
          } catch (edgeError) {
